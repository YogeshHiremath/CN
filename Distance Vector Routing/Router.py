...
class Router:
	...
	def ReadNetworkVectors():
		...
		t3 = threading.Thread(target=Router.UpdateNetworkVectors, args=(message.split(":".encode('utf-8')), addr))
		t3.start()
		...
		for i in range(Router.neighbours.__len__()):
			...
			for j in range(Router.vectorList.__len__()):
				if Router.neighbours[i] == Router.nextHopList[j]:
					data = data + str(float("inf")).encode('utf-8')
				else:
					data = data + str(Router.vectorList[j]).encode('utf-8')
				if j + 1 != Router.vectorList.__len__():
					data = data + ":".encode('utf-8')
			port = Router.portNumbers[Router.IndexOf(Router.neighbours[i])]
			Router.sock.sendto(data, ("127.0.0.1".encode('utf-8'), port))
